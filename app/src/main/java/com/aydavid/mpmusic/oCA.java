...
public class oCA extends ArrayAdapter<song> {
	...
	public View getView(int position, View view, ViewGroup parent)
	{
		...
		if (position !=0){
			...
			IndexLabel = ""+ ( (defaultTab_id == R.id.tab_mostplayed)?(mostPlaysTimes.get(mostPlaysTimes.size()-position)):position );  //use number of plays or regular position
			...
			try{
				...
				vHolder.songArtist.setText(IndexLabel+"| "+"  "+" "+songAt.getSongArtist());
				...
				public void  onClick(View v){
					new Thread(new Runnable() {    @Override
						public void run(){
							...
							bufSongList.load();
							filtering("", Playerprops.SONG_LIST_ALL);
					}}).start();
				}
			});
